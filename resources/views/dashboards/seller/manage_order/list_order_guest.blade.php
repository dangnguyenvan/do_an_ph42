...
@section('title', 'List Order Guest')
@section('content')
<h1>List Order Guest</h1> 
...
<div class="row">
    <div class="col-3">
        ...
        <div id="form_css">
            <form action="{{ route('seller.order-guest.search_by_status') }}" method="get">
    ...
    <div class="col-3">
        ...
        <br>
        <br>
        <form action="{{ route('seller.order-guest.search_by_code') }}">
            <div class="form-inline">
    ...
    <div class="col-6">
        ...
        <div class="form_css">
            <form action="{{ route('seller.order-guest.search_by_date') }}">
                <div class="form-inline">
...
<table id="product-list" class="table table-bordered table-hover table-striped">
    ...
    <tbody>
        ...
        <tr>    
            ...
            <td>{{$od->name}}</td>
            <td>{{$od->order_date}}</td>
            <td><a href="{{ route('seller.order-guest.order_detail',$od->id) }}">view order detail</a></td>
            <td>{{$od->status}}</td>
            <td>
                
                <form action="{{ route('seller.order-guest.update_status_order', $od->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <select name="status" class="form-control">
                        @if(!empty($order_status))
